...
?>
	<div class="wrap proofreading-backend proofreading-<?php echo $current_page ?>">
		<span class="clearfix proofreading-title">
			<span class="proofreading-logo"><img src="<?php echo plugins_url('../images/logo.png', __FILE__) ?>"></span>
			<h1><?php echo esc_html__('Proofreading', 'proofreading') ?></h1>
		</span>
		
		<h2 class="nav-tab-wrapper"> 
			<a href="options-general.php?page=<?php echo PROOFREADING_PLUGIN_SLUG ?>" class="nav-tab <?php echo ($current_page == 'settings') ? 'nav-tab-active' : '' ?>">
				<span class="dashicons dashicons-translation" aria-hidden="true"></span><?php echo esc_html__('Languages', 'proofreading') ?>
			</a> 
			<a style="color:#88C" href="options-general.php?page=<?php echo PROOFREADING_PLUGIN_SLUG ?>&subpage=about" class="nav-tab <?php echo ($current_page == 'about') ? 'nav-tab-active' : '' ?>">
				<span class="dashicons dashicons-info-outline" aria-hidden="true"></span><?php echo esc_html__('About', 'proofreading') ?>
			</a>
		</h2>
	...
	if (isset($_POST['submit'])) {
		...
		<div id="setting-error-settings_updated" class="<?php echo $res ? '' : 'error' ?> updated settings-error notice is-dismissible"> 
			...
			<button type="button" class="notice-dismiss">
				<span class="screen-reader-text"><?php echo esc_html__('Dismiss this notice.', 'proofreading') ?></span>
			</button>
		</div>
<?php }
	else
...
?>
	...
	<form method="post" novalidate="novalidate">
		...
		<table class="form-table">
			<tbody>
				<tr>
					<th scope="row"><label for="proofreading-language-default"><?php echo esc_html__('Default correction Language', 'proofreading') .':' ?></label></th>
					<td>
						<?php global $wpdb;
						...
						<select id="proofreading-language-default" name="proofreading-language-default">
							<?php $current_value = get_option( "proofreading-language-default", 2 );
								...
								<option <?php echo $current_value == $language['code'] ? 'selected' : '' ?> value="<?php echo $language['code'] ?>"><?php echo $language['name'] ?></option>
							<?php endforeach; ?>
						</select>
					</td>
				</tr>
				...
				<tr>
					<th scope="row">
						<label><?php echo esc_html__('Rules category', 'proofreading') ?></label>
						<p class="description" id="tagline-description"><?php echo esc_html__('Enable or disable types of rule to be checked in the corrections.', 'proofreading') ?></p>
						<p><?php echo esc_html__('Select the language to manage the rules:', 'proofreading') ?></p>
						...
						<table class="proofreading_rules">
							<thead>
								<tr class="select_all_rules_row" style="display:none">
									<td><label for="select_all_rules"><input type="checkbox" id="select_all_rules"> <strong><?php echo esc_html__('Select all rules', 'proofreading') ?><strong></label></td>
								</tr>
							</thead>
		...
		<p class="submit"><input type="submit" name="submit" id="submit" class="button button-primary" value="<?php echo esc_html__('Save settings', 'proofreading') ?>"></p>
	</form>
...
function proofreading_admin_page_about_handler(){ ?>
	...
	<table id="proofreading_about_support">
		<tr>
			<td class="scribit_support_description"><?php echo esc_html__('If you like our plugin please feel free to give us 5 stars :)', 'proofreading') ?></td>
			<td><a target="_blank" class="button button-primary scribit_support_button" rel="nofollow" href="https://wordpress.org/support/plugin/proofreading/reviews/">
				<span style="color:#CFC" class="dashicons dashicons-star-filled" aria-hidden="true"></span><?php echo esc_html__('WRITE A PLUGIN REVIEW', 'proofreading') ?>
			</a></td>
		</tr>

		<tr>
			<td class="scribit_support_description"><?php echo esc_html__('If you want to help us to improve our service please Donate a coffe', 'proofreading') ?></td>
			<td><a target="_blank" class="button button-primary scribit_support_button" rel="nofollow" href="https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=riccardosormani@gmail.com&item_name=Proofreading Wordpress plugin donation&no_note=0">
				<span style="color:#FC9" class="dashicons dashicons-coffee" aria-hidden="true"></span><?php echo esc_html__('DONATE WITH PAYPAL', 'proofreading') ?>
			</a></td>
		</tr>

		<tr>
			<td class="scribit_support_description"><?php echo esc_html__('If you want some information about our Company', 'proofreading') ?></td>
			<td><a target="_blank" class="button button-primary scribit_support_button" href="mailto:wordpress@scribit.it">
				<span style="color:#DDD" class="dashicons dashicons-email" aria-hidden="true"></span><?php echo esc_html__('CONTACT US', 'proofreading') ?>
			</a></td>
		</tr>
	</table>
